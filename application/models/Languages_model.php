...
class Languages_model extends CI_Model{
    ...
    public function read(){
        $results=$this->database_model->read($this->table_name,array('id!='=>0),array('id','asc'));
        return count($results)>0?$results:false;
    }
    
    public function read_ids($data){
        $results=$this->database_model->read_in($this->table_name,'id',$data);
        return count($results)>0?$results:false;
    }
